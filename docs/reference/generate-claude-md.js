// ═══════════════════════════════════════════════════════════════
// IdeaForge CLAUDE.md Generator — Template Engine
// Takes configuration object → produces complete CLAUDE.md string
// ═══════════════════════════════════════════════════════════════

export function generateClaudeMD(cfg) {
  const {
    direction = "",
    principles = [],
    // Business model
    bizType = "digital", bizTeam = "solo_ai", bizAI = "native",
    bizTouch = "self_serve", bizInvest = "bootstrap",
    // Targets
    revenue = 5000, budget = "low",
    // Generation
    ideasPer = 15, deckCut = 80, novelty = 70,
    mix = { trend: 6, pattern: 4, wild: 4 },
    // Sections toggles
    sections = { profile: true, team: true, scoring: true, feed: true, verticals: true },
    // Profile
    founderName = "Ofir Kedmi", founderRole = "Creative Director + Product Strategist",
    founderTime = "1 day/week",
    skills = [], filters = [],
    // Team
    founders = [], advisors = [],
    // Verticals
    verts = [], focusMode = null,
    // Scoring
  ...
  const L = {
    ...
    budget: { zero: "$0 — organic only", low: "$100-500/month", medium: "$500-2K/month", high: "$2K+/month" },
  };
  ...
  const allConstraints = [
    ...autoRejects.map(r => `- ${r} → REJECT`),
    ...constraints.filter(c => c.on).map(c => `- ${c.t} → REJECT`),
  ];
...
`;
    ...
    md += `## Operator Profile ✗ (excluded — no operator constraints applied)
...
\`\`\`
DAILY:
Inbox → Trend Scouts → Competitor Scout → Creative Founders (${ideasPer} ideas) → Advisors → [Iteration] → Pitch Decks (${deckCut}%+) → Daily Report
...
${pendingFeed.map(f => {
  const types = { idea: "💡 IDEA SEED", signal: "📰 SIGNAL", data: "📊 DATA POINT", url: "🔗 URL TO MINE" };
  return `- **${types[f.type] || "ITEM"}:** ${f.t}${f.vertical ? ` [${f.vertical}]` : ""}`;
}).join("\n")}
...
${bizType === "physical" || bizType === "hybrid" ? "7. Scout physical product opportunities, e-commerce trends, and supply chain innovations" : ""}
${novelty >= 70 ? "7. Actively seek trends in UNEXPLORED verticals — avoid retreading familiar territory" : ""}
...
`;
    ...
    // Group by category
    const catLabels = {
      serial: "🎯 THE SERIAL ENTREPRENEURS (Pattern Recognition)",
      creative: "🎨 THE CREATIVE OUTSIDERS (Fresh Perspectives)",
      operator: "🚀 THE OPERATORS (Execution Focus)",
      gtm: "📣 THE GTM SPECIALISTS (Go-To-Market & AI Marketing)",
      futurist: "🔥 THE FUTURIST (Trend Translation)",
    };
    const catIntros = {
...
`;
    ...
    // Ideation process
    md += `### 💡 COLLABORATIVE IDEATION PROCESS
...
|----------|-------|-------------|
| **🔥 Trend-Driven** | ${mix.trend} | Tied to something new in last 48h |
| **🎯 Pattern-Based** | ${mix.pattern} | Based on proven market patterns |
| **🎲 Wild Cards** | ${mix.wild} | Unexpected, creative, contrarian |
...
`;
    ...
    md += `## Creative Founders ✗ (excluded — simplified generation without persona orchestration)
...
${bizTouch === "self_serve" ? "| **Cold email/LinkedIn** | ❌ Not allowed | — | — |" : "| **Cold outreach** | ⚠️ Use sparingly | Weeks | B2B |"}
${bizType === "physical" ? "| **Amazon/Etsy Marketplace** | ✅ AI listings | Days | Physical products |\n| **Shopify + Ads** | ✅ AI creative | Days | E-commerce |" : ""}
...
| Condition | Result |
|-----------|--------|
${bizTouch === "self_serve" ? "| Requires ANY manual sales/demos | → REJECT |\n" : ""}| GTM is "SEO" only with no fast channel | → CAP at 70% |
...
`;
    ...
    md += `## Scoring ✗ (excluded — no structured scoring applied)
...
### Red Flags (Penalties)
${bizTouch === "self_serve" ? "- Requires any manual sales → REJECT\n" : ""}- Acquisition "figure out later" → -20%
- Single acquisition channel → -10%
- Oversaturated market → -15%
${bizTouch === "self_serve" || bizTouch === "low" ? "- High-touch onboarding → -15%\n" : ""}
### Bonuses
...
4. If v2 ≥ ${deckCut}% → auto pitch deck
...
8. **🚨 PITCH DECKS** (20 min): Generate decks for **TOP 6** ideas scoring ${deckCut}%+ (see below)
...
`;
  ...
  md += `## ⚠️ MANDATORY: Automatic Pitch Deck Generation
...
| ${deckCut}-89% | STRONG CANDIDATE | ✅ **Include in TOP 6** |
| ${deckCut - 15}%-${deckCut - 1}% | — | Iterate first, no deck unless reaches ${deckCut}%+ |
| <${deckCut - 15}% | — | No deck |
...
[If ${deckCut}-89%: ⭐ STRONG CANDIDATE: [Name] - [X]% - [One-liner]]
...
*v4.0 — Generated by IdeaForge · ${date} · TOP 6 deck cap, wave-based generation, novelty at ${novelty}%*
`;
